...
const endpointModeSelect = document.getElementById('endpointModeSelect');
...
let currentEndpointMode = 'daily';

async function loadEndpoints() {
  if (!endpointModeSelect) return;

  try {
    const data = await fetchJson('/admin/endpoints');
    currentEndpointMode = data.mode || 'daily';
    endpointModeSelect.value = currentEndpointMode;
    setStatus(`当前模式: ${getModeLabel(currentEndpointMode)}`, 'success', endpointStatusEl);
  } catch (e) {
...
function getModeLabel(mode) {
  const labels = {
    ...
    'round-robin': '轮询(全部)',
    'round-robin-dp': '轮询(D+P)'
  };
  return labels[mode] || mode;
}

async function switchEndpointMode() {
  if (!endpointModeSelect || !switchEndpointBtn) return;

  const selectedMode = endpointModeSelect.value;
  if (selectedMode === currentEndpointMode) {
    setStatus('当前已是该模式，无需切换', 'info', endpointStatusEl);
    return;
  }
  ...
  try {
    ...
    currentEndpointMode = result.mode || selectedMode;
    setStatus(`✓ 已切换至 ${getModeLabel(currentEndpointMode)}`, 'success', endpointStatusEl);
    ...
    switchEndpointBtn.textContent = '🔄 切换';
    switchEndpointBtn.disabled = false;
  }
}

if (switchEndpointBtn) {
  switchEndpointBtn.addEventListener('click', switchEndpointMode);
}
